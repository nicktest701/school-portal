...
const processWeeklyAttendance = (attendanceData) => {
    ...
    attendanceData.forEach(({ level, date, status }) => {
        ...
        if (!weeklyAttendance[formattedDate][level]) {
            weeklyAttendance[formattedDate][level] = { Present: 0, Absent: 0, Unmarked: 0 };
        }
    ...
    ["Present", "Absent", "Unmarked"].forEach((status) => {
        classLevels.forEach((level) => {
            datasets.push({
                label: `${level} - ${status}`,
                data: labels.map((date) => weeklyAttendance[date]?.[level]?.[status] || 0),
                backgroundColor:
                    status === "Present"
                        ? "#012e54"
                        : status === "Absent"
...
const getAttendanceByGender = (attendances) => {
    ...
    const presentData = genders.map((gender) => attendanceByGender[gender].Present);
...
module.exports = {
    ...
    getMonthsBetween, processFeeForTerm, processWeeklyFees, getTotalFeesForWeek,
    processWeeklyAttendance, getTotalAttendance, getAttendanceByGender, processGeneralWeeklyAttendance
}
